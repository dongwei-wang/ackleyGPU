...
__global__ ackley_kernel(double *x, double *sum1, double *sum2, int dim){

	__shared__ sm[BLOCK_SIZE];
	__shared__ sm_cos[BLOCK_SIZE];

	int global_tid = blockDim.x * blockIdx.x + threadIdx.x;
	int tid = threadIdx.x;

	// anotherz = x[i] - Ovector[i]
	sm[tid] = 0-x[global_tid];
	...
	// lambda
	sm[tid] = sm[tid] * pow( 10, 0.5* tid/(double)(dim-1) );
